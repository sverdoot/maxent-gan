...
from soul_gan.models.utils import load_gan, estimate_lipschitz_const
from soul_gan.models.studiogans import StudioDis, StudioGen
from soul_gan.utils.general_utils import DotConfig, CONFIGS_DIR  # isort:block
...
def main(args):
    ...
    if args.all:
        config_paths = [_.as_posix() for _ in Path(CONFIGS_DIR, 'gan_configs').glob('*')]
    else:
        config_paths = args.configs

    if args.exc:
        config_paths = sorted(list(set(config_paths) - set([Path(_).resolve().as_posix() for _ in args.exc])))
    ...
    for config_path in config_paths:
        ...
        for thermalize in [True, False]:
            ...
            log_norm_const = estimate_log_norm_constant(gen, dis, args.n_pts_mc, batch_size=args.batch_size, verbose=True)
            print(f'\t log norm const: {log_norm_const:.3f}')

            lipschitz_const = estimate_lipschitz_const(gen, dis, args.n_pts_lipschitz, batch_size=args.batch_size, verbose=True)
            print(f'\t lipschitz const: {lipschitz_const:.3f}')

            if args.upd_config:
                raw_config['gan_config']['thermalize'][thermalize]['log_norm_const'] = log_norm_const
                raw_config['gan_config']['thermalize'][thermalize]['lipschitz_const'] = lipschitz_const
