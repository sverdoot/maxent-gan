...
def load_gan(config: DotConfig, device: torch.device) -> Tuple[torch.nn.Module, torch.nn.Module]:
    gen = ModelRegistry.create_model(config.generator.name, **config.generator.params).to(device)
    state_dict = torch.load(Path(config.generator.ckpt_path, map_location=device))
    gen.load_state_dict(state_dict)

    dis = ModelRegistry.create_model(config.discriminator.name, **config.discriminator.params).to(device)
    state_dict = torch.load(Path(config.discriminator.ckpt_path, map_location=device))
...
def main(config, gan_config, device):
    ...
    if 'sample' in config:
        save_dir = Path(config.sample.save_dir,  datetime.datetime.now().strftime("%Y_%m_%d_%H_%M"))
        save_dir.mkdir(exist_ok=True, parents=True)

        feature = FeatureFactory.create_feature(config.sample.feature.name, **config.sample.feature.params)

        z_dim = gen.z_dim
        proposal = torch.distributions.multivariate_normal.MultivariateNormal(torch.zeros(z_dim).to(device), torch.eye(z_dim).to(device))
    ...
    if 'compute_fid' in config:
        pass

    if 'compute_is' in config:
        pass
