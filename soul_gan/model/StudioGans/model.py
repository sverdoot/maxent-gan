...
def load_generator_discriminator(
    DATA, OPTIMIZATION, MODEL, STYLEGAN2, MODULES, RUN, device, logger
):
    if device == 0:
        logger.info("Build a Generative Adversarial Network.")
    module = __import__(
        "models.{backbone}".format(backbone=MODEL.backbone),
        fromlist=["something"],
    )
    if device == 0:
        logger.info(
    ...
    if MODEL.backbone == "stylegan2":
        channel_base = (
            32768 if DATA.img_size >= 512 or DATA.name == "CIFAR10" else 16384
        )
        gen_c_dim = DATA.num_classes if MODEL.g_cond_mtd == "cAdaIN" else 0
        dis_c_dim = (
            DATA.num_classes if MODEL.d_cond_mtd in STYLEGAN2.cond_type else 0
        )
        ...
        if MODEL.apply_g_ema:
            ...
            Gen_ema_mapping, Gen_ema_synthesis = (
                Gen_ema.mapping,
                Gen_ema.synthesis,
            )
            ...
            ema = EmaStylegan2(
                ...
                effective_batch_size=OPTIMIZATION.batch_size
                * OPTIMIZATION.acml_steps,
            )
        else:
            Gen_ema, Gen_ema_mapping, Gen_ema_synthesis, ema = (
                None,
                None,
                None,
                None,
...
def prepare_parallel_training(
    ...
    if distributed_data_parallel:
        ...
        if MODEL.backbone == "stylegan2":
            Gen_mapping = DDP(
                Gen.mapping, device_ids=[device], broadcast_buffers=False
            )
            Gen_synthesis = DDP(
            ...
            Gen = DDP(
                Gen, device_ids=[device], broadcast_buffers=synchronized_bn
            )
        Dis = DDP(
            Dis,
            device_ids=[device],
        ...
        if apply_g_ema:
            if MODEL.backbone == "stylegan2":
                Gen_ema_mapping = DataParallel(
                    Gen_ema.mapping, output_device=device
                )
