...
from soul_gan.models.base import ModelRegistry, BaseDiscriminator, BaseGenerator
...
class StudioGen(BaseGenerator):
    def __init__(self, mean, std, config):
        ...
        module = __import__("models.{backbone}".format(backbone=cfg.MODEL.backbone), fromlist=["something"])
        self.gen = module.Generator(z_dim=cfg.MODEL.z_dim,
    ...
    def load_state_dict(self, state_dict, strict: bool = True):
        return self.gen.load_state_dict(state_dict['state_dict'], strict=strict)
...
class StudioDis(BaseDiscriminator):
    def __init__(self, mean, std, output_layer, config):
        ...
        module = __import__("models.{backbone}".format(backbone=cfg.MODEL.backbone), fromlist=["something"])
        self.dis = module.Discriminator(img_size=cfg.DATA.img_size,
