...
def prepare_2d_gaussian_grid_data(
    ...
    """
    assert (n_modes**.5).is_integer()
    dataset = []
    n_modes_per_ax = int(n_modes **.5)
    mesh_modes = np.meshgrid(np.linspace(xlims[0], xlims[1], n_modes_per_ax), np.linspace(ylims[0], ylims[1], n_modes_per_ax))
